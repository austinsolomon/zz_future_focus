...
(2,"Origins — Why rules don't scale","Why exhaustively hand-listing rules for real-world tasks is hopeless: the cases and exceptions blow up past any hope of enumeration.","Combinatorial explosion","The explosion of cases in rule-based systems is a core reason the field turned to learning rules from data.",["bigpicture","why"],1969),
...
(1,"Origins — The 1940s computer","The room-sized 1940s electronic machine built to compute artillery firing tables — the icon of the computer as a fast, fixed calculator.","ENIAC","ENIAC (1945) executed fixed arithmetic at speed; 'intelligence' wasn't on the menu — it did exactly what it was wired to do.",["bigpicture","history"],1946),
(1,"Origins — What 1950s–60s computers did","How early business computers ran: jobs like payroll and census were queued and executed in bulk with no interaction, following fixed instructions exactly.","Batch processing","Business 'data processing' dominated; the computer was a meticulous clerk, not a learner.",["bigpicture","history"],1955),
(2,"Origins — The hardest 'easy' problems","The observation that tasks easy for humans — seeing, hearing, moving — are extremely hard for machines, while abstract reasoning is comparatively easy.","Moravec's Paradox","Perception resists explicit rules; this paradox is part of why AI turned toward learning from data.",["bigpicture","why"],1988),
(2,"Origins — Why expert systems stalled","The slow, incomplete slog of extracting human experts' knowledge into hand-written rules — the wall that capped 1980s rule-based AI.","Knowledge-acquisition bottleneck","This bottleneck motivated systems that infer their own rules from data instead of being hand-programmed.",["bigpicture","why"],1984),
(1,"Origins — The brain-inspired school","The approach to AI in which intelligence emerges from networks of simple units that adjust their connection strengths — loosely imitating the brain.","Connectionism","Connectionism gave neural networks their name and design; it rivalled symbolic AI and ultimately powered deep learning.",["bigpicture","why"],1986),
],
...
(1,"Foundations — Unsupervised Learning","Finding structure in data that has no labels — grouping similar items together.","Unsupervised Learning","Clustering (e.g. k-means) and dimensionality reduction are the classics; the model discovers patterns instead of predicting given answers.",["ml","basics"],1990),
(2,"Foundations — Reinforcement Learning","Learning by trial and error: an agent takes actions in an environment to maximize reward.","Reinforcement Learning","The agent observes a state, acts, and receives a reward; over time it learns a policy. This loop later became the backbone of RLHF for aligning LLMs.",["ml","basics","rl"],1992),
...
(3,"Vision & Sequence — GRU vs LSTM","Why a GRU is often preferred despite the LSTM's longer track record.","Parameter efficiency","The GRU merges gates and drops the separate cell state, training faster with comparable accuracy.",["rnn"],2014),
...
(4,"Vision & Sequence — Why CNNs Beat Dense Nets on Images","The two CNN properties that make them vastly more efficient than fully connected nets for pixels.","Spatial inductive bias","They exploit spatial locality and reuse filters, cutting parameters by orders of magnitude and improving generalization.",["cnn","theory"],1989),
...
(4,"Vision & Sequence — Sequential Bottleneck of RNNs","The structural reason RNNs are slow to train on modern hardware.","Sequential bottleneck","Each step depends on the previous one, so timesteps can't be parallelized — a key motivation for the Transformer.",["rnn","limitation"],2017),
...
(4,"Vision & Sequence — Image Captioning Bridge","The task that fused a CNN encoder with an RNN decoder to describe images in words.","Image captioning","Show-and-Tell (2015) used a CNN to encode the image and an LSTM to generate the caption — an early multimodal pipeline.",["cnn","rnn"],2015),
...
(2,"Representations — Embedding API (Applied)","How a developer turns text into vectors in practice without training a model from scratch.","Embedding API","Call a hosted endpoint (e.g. OpenAI text-embedding, Cohere, or open models like BGE/E5) to get a fixed-length vector per text, then compare with cosine similarity. This is the practical on-ramp to semantic search and RAG.",["embeddings","apply"],2022),

# ---- L3 ----
(3,"Representations — Word2Vec training mode","The Word2Vec mode that predicts the surrounding context words from a single target word — slower, but better on rare words.","Skip-gram","Its counterpart, CBOW, predicts the target word from its context and trains faster.",["embeddings"],2013),
...
(3,"Representations — Exa (Applied: GTM lead-gen)","A neural, embeddings-based search API — effectively a vector database over the web — you query in plain language to find entities by meaning.","Exa","In GTM engineering Exa powers lead generation: a query like 'companies similar to Stripe that are hiring ML engineers' returns a semantically matched list of prospects to enrich into your CRM — meaning-based search instead of brittle keywords.",["vectordb","gtm","apply"],2023),
(3,"Representations — Pinecone / pgvector (Applied)","The managed and Postgres-native ways to store and query embeddings in production.","Pinecone / pgvector","To apply embeddings you need a store: managed vector DBs (Pinecone, Weaviate, Qdrant) or pgvector, which adds a vector column to Postgres. You 'upsert' {id, vector, metadata}, then query by nearest neighbor.",["vectordb","apply","tools"],2021),

# ---- L4 ----
(4,"Representations — How Attention Fixed Seq2Seq","Why attention removed the fixed-vector bottleneck in translation.","Soft alignment","The decoder weights every encoder state per output token, so long inputs no longer get crushed into one vector.",["attention","theory"],2014),
(4,"Representations — Why Scale by √dₖ","The reason scaled dot-product attention divides scores before softmax.","Scaling by √dₖ","Large dot products in high dimensions push softmax into tiny-gradient regions; the √dₖ factor counteracts this.",["attention","theory"],2017),
...
(4,"Representations — RAG Pipeline","The end-to-end flow connecting a vector store to an LLM at query time.","RAG pipeline","The query is embedded, top matches are fetched from the vector DB, then inserted into the prompt as context.",["retrieval","llm"],2020),
...
(2,"Transformers & LLMs — System Prompt (Applied)","The persistent instruction that sets an assistant's role, rules and tone for a whole conversation.","System Prompt","It's the first lever you tune when applying an LLM: define the persona, constraints and output format here rather than repeating them in every user message.",["llm","apply","prompt"],2022),
...
# ---- L3 ----
(3,"Transformers & LLMs — GPT's architecture","The transformer design used by GPT: each position attends only to earlier tokens, so the model generates text left-to-right.","Decoder-only","Contrast with BERT's encoder, which reads bidirectionally for understanding rather than generation.",["transformers","theory"],2018),
...
(3,"Transformers & LLMs — Examples in the prompt","Prompting that includes a handful of worked examples to steer the model's format and behaviour, with no weight updates.","Few-shot prompting","Contrast with zero-shot, which relies on instructions alone; both work purely at inference time.",["llm"],2020),
(3,"Transformers & LLMs — Building with an LLM API (Applied)","The practical way most apps use a model today without ever training one.","LLM API Call","Send a prompt (plus an optional system message and tool definitions) to a hosted model and parse the completion. The applied skill is prompt + context design, not training — this is how non-technical builders ship AI features.",["llm","apply"],2023),
(3,"Transformers & LLMs — Keeping answers factual (Applied)","Supplying an LLM with retrieved, up-to-date documents at query time so its answers stay factual — preferred over retraining when knowledge keeps changing.","Grounding","Grounding via retrieval (RAG) is cheap and instantly updatable; fine-tuning is for changing style or teaching a narrow skill.",["llm","rag","apply"],2023),

# ---- L4 ----
(4,"Transformers & LLMs — Why Transformers Beat RNNs","The two advantages that let transformers replace recurrent nets for sequences.","Parallelization","Self-attention processes all tokens at once and connects distant positions in one hop, fixing RNN slowness and forgetting.",["transformers","theory"],2017),
...
# ---- L5 ----
(5,"Transformers & LLMs — Transformer Block Anatomy","The repeating sub-layers that make up a single transformer layer.","Transformer block","Each block = multi-head self-attention then a feed-forward net, each wrapped with residual connections and layer normalization.",["transformers","theory"],2017),
(5,"Transformers & LLMs — Kaplan vs Chinchilla","The shift in scaling guidance between the 2020 and 2022 results.","Token–parameter balance","Kaplan emphasized parameter scaling; Chinchilla corrected it, showing tokens should grow about proportionally with parameters.",["llm","theory"],2022),
(5,"Transformers & LLMs — RLHF Mechanics","The two-model loop that turns human rankings into a training signal.","Reward modeling","A reward model learns from human preference rankings, then PPO fine-tunes the LLM to maximize that reward.",["llm","alignment"],2022),
